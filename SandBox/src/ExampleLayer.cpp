    ...
    :Layer("Example layer"), m_CameraController(1280.0f / 720.0f, true)
{
    ...
    std::dynamic_pointer_cast<SoLin::OpenGLShader>(m_TextureShader)->UploadUniformFloat("u_TilingFactor", 1.0f);
    std::dynamic_pointer_cast<SoLin::OpenGLShader>(m_TextureShader)->UploadUniformFloat4("u_Color", {1.0f,1.0f,1.0f,1.0f});
...
void ExampleLayer::OnUpdate(SoLin::Timestep ts) {
    ...
    m_Texture->Bind();
    SoLin::Renderer::Submit(m_TextureShader, m_SquareVA, glm::translate(glm::mat4(1.0f),glm::vec3(0.0f,0.0f,0.2f)) * glm::scale(glm::mat4(1.0f), glm::vec3(1.5f)));

    SoLin::Renderer::Submit(m_Shader, m_VertexArray, glm::scale(glm::mat4(1.0f), glm::vec3(2.5f)));
...
void ExampleLayer::OnEvent(SoLin::Event& event){
